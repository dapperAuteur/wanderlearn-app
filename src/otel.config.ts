...
// OpenTelemetry → Honeycomb tracing. Loaded from src/instrumentation.ts's register() on both
// runtimes, BEFORE the Sentry configs, because whoever registers the global tracer provider first
// wins and this file must be the winner (see skipOpenTelemetrySetup in sentry.server.config.ts).
//
...
/**
 * Uptime monitors probe /api/health around the clock. Those requests are pure noise in Honeycomb —
 * same span, thousands of times a month, across every monitored app — so they are dropped at the
 * sampler, before they ever count against the free tier's 20M events. Everything else is recorded
 * unsampled; current traffic sits far below the ceiling, and the agreed lever if that changes is
 * ratio sampling, not code changes (witus plan 30 §7.4).
 */
