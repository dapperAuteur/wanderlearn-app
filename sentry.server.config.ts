...
if (dsn) {
  Sentry.init({
    ...
    tracesSampleRate: 0,
    // Tracing belongs to @vercel/otel → Honeycomb (src/otel.config.ts). Sentry v8+ installs its
    // own OpenTelemetry provider by default even at tracesSampleRate 0; two global providers race
    // and the loser silently drops its spans. Error capture does not need a provider, so skipping
